...
- `dashboard.html`: ترتيب عمودي منظم + صلاحيات على العناصر الجديدة والشريط السفلي.
- إخفاء شريط الأزرار العشوائي القديم المعتمد على flex-wrap.
- هوية بصرية متقاربة مع شاشة الدخول (تيل).

## 4.64.0 — STEP 64: بداية عام دراسي جديد

- تفعيل زر **بداية عام دراسي جديد** في لوحة التحكم (مسؤول عام).
- معالج: معاينة الإحصاءات → اختيار العام → كلمة المرور → إقرار + تأخير.
- SQL: `preview_new_academic_year` + `start_new_academic_year`
- أرشفة الطلاب النشطين اختيارياً دون حذف السجلات السلوكية.
- جدول `academic_year_transitions` لتوثيق كل انتقال.
- لا يمس الحسابات ولا الكتالوجات ولا المخالفات التاريخية.

## 4.63.0 — STEP 63: System Factory Reset

- إضافة منطقة خطر في لوحة التحكم للمسؤول العام.
- زر **تنظيف جميع بيانات النظام** مع معالج متعدد الخطوات:
  1. تحذير صريح بما يُمسح وما يبقى
  2. إعادة مصادقة بكلمة المرور
  3. كود تحقق لمرة واحدة (OTP) عبر Edge Function + بريد (Resend اختياري)
  4. خانة إقرار + تأخير 10 ثوانٍ قبل التنفيذ
- SQL: جداول `system_reset_tokens` ودوال `request_system_reset_otp` / `admin_issue_system_reset_otp` / `execute_system_factory_reset`
- مسح Storage لملفات `student-files` عبر Edge Function بعد نجاح مسح القاعدة
- لا يُرجع الكود الصريح إلى المتصفح؛ الإصدار عبر service role فقط
- زر «بداية عام دراسي جديد» مُجهّز كعنصر واجهة (معطّل — قريباً)

- 4.62.20: إصلاح تعطل لوحة التحليلات عند عدم وجود زر «فحص نطاق الصلاحيات» بسبب اختلاف/كاش نسخة HTML وJS؛ تمت إضافة فحوصات آمنة للعناصر الاختيارية وcache-busting للملفات.
- 4.62.19: إضافة فحص آمن وتجميعي لنطاق صلاحيات التحليلات، مع زر داخل لوحة التحليلات يوضح إجمالي الفترة والمستبعد حسب المرحلة/إسناد الفصل دون كشف بيانات خارج النطاق.
...
- إضافة RPCs: `list_my_manual_whatsapp_confirmations` و `confirm_manual_whatsapp_send`.
- الحفاظ على الأنماط الثلاثة للرسائل وربط التأكيد بتاريخ ونمط الرسالة.
- ترحيل أفضل جهد للتأكيدات القديمة الموجودة محليًا.
- تحديث واجهة الإشعارات لتوضيح أن قاعدة البيانات هي المرجع المشترك.

# 4.62.10 — Smart WhatsApp Notification Center

- تحسين مركز إشعارات WhatsApp بعد نجاح اختبار 3 مخالفات.
- إضافة إرسال «المحدد» وإرسال «جميع المعلّقين» بشكل منفصل.
- معاينة الرسالة الفعلية بالنمط المختار بدل عرض رسالة الطابور فقط.
- فصل حالة التأكيد اليدوي حسب التاريخ + نوع الرسالة + الإشعار.
- إظهار سبب عدم الإرسال عندما لا يوجد رقم صالح.
- تحسين العدادات والحالات النصية.
- الحفاظ على الفرق بين إشعار الاطلاع والتنبيه الكامل والتنبيه مع التصعيد.
- تحديث الإصدار إلى 4.62.10.
...
- إشعار الاطلاع يوجه ولي الأمر إلى سلوكي باستخدام الرقم القومي وكود الطالب ولا يرسل التقرير الكامل.
- التنبيه الكتابي يمكن إرساله كاملاً، مع خيار إضافة ملخص التصعيد.
- اختيار عدة طلاب يدوياً قبل بدء الإرسال، مع إعادة استخدام نفس نافذة WhatsApp.
- لا يعتمد هذا المسار على Meta Cloud API.
# 4.62.7 — تثبيت Baseline الرسمي للتطوير القادم

- هذه النسخة مبنية على 4.62.6 دون تغيير وظيفي في مسار الإشعارات اليدوي.
- تحديث `BASELINE_VERSION.txt` و`APP_VERSION` إلى 4.62.7.
- تحديث أرقام cache-busting إلى 4.62.7 لتفادي تحميل ملفات JavaScript القديمة من المتصفح.
- إضافة `BASELINE-4.62.7.md` كنقطة انطلاق رسمية للتعديلات القادمة.

# 4.62.6 — إصلاح تاريخ الإشعارات والإرسال اليدوي عبر نفس نافذة واتساب

- إصلاح حساب «اليوم» في صفحة الإشعارات وتسجيل المخالفات ليستخدم التاريخ المحلي بدلاً من UTC؛ هذا يمنع ظهور «0 إشعار / تخطّي» بعد منتصف الليل في مصر بسبب انتقال التاريخ إلى اليوم السابق.
- تحسين الإرسال اليدوي الجماعي: بعد فتح واتساب لأول مرة، يعاد استخدام نفس نافذة واتساب لكل طالب بدلاً من محاولة إنشاء نافذة جديدة.
- الإرسال اليدوي يظل يفتح رسالة جاهزة عبر `wa.me`؛ الإرسال الفعلي يتم بالضغط على زر الإرسال داخل واتساب، ولا يُعتبر نجاح API.
- تحديث رقم إصدار الواجهة إلى 4.62.6.

## 4.62.5 — إرسال واتساب يدوي جماعي من رقم الأخصائي
- wa.me في نفس النافذة + مسار السابق/التالي للمعلّق.
- تعطيل مسار API من الواجهة مؤقتاً.

## 4.62.4 — إصلاح SoloukiSession في صفحة الإشعارات
- تحميل `js/core/session.js` وباقي سكربتات النواة في `notifications.html`.

## 4.62.3 — إصلاحات واجهة ملف الطالب والطباعة
- تسمية كود الطالب بدل رقم الجلوس في ملف السلوك والطباعة.
- إزالة زر الطباعة غير الضروري من نافذة ملف الطالب (الطباعة الرسمية من ملف السلوك الكامل).
...
## 4.62.11 — تحسين قوالب واتساب والمعاينة قبل الإرسال
- فصل واضح بين إشعار الاطلاع على سلوكي والتنبيه الكتابي والتنبيه مع خطوات المتابعة والتصعيد.
- إضافة معاينة كاملة قبل فتح WhatsApp.
- السماح بمراجعة/تعديل نص الرسالة الحالية فقط قبل الإرسال.
- تحسين صياغة إشعار الاطلاع: الرقم القومي + كود الطالب + رابط سلوكي دون تفاصيل المخالفة.
